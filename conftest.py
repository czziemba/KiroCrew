...
#: Thread names already reported on this worker. A leak damages every LATER test, so
#: reporting each one only once keeps a single defect from erroring the rest of the
#: shard -- but keyed on the NAMES, not a bare flag: a second, differently-named
#: exporter is a different defect, and a bool would let it through silently while the
#: run showed one error for the first.
_telemetry_leaks_reported: set[str] = set()
...
def _no_leaked_telemetry_exporter():
    ...
    letting it stand would turn one defect into a red suite and hide which test
    actually caused it.
    """
    yield
    live = [
    ...
    fresh = names - _telemetry_leaks_reported
    _telemetry_leaks_reported.update(names)
    if not fresh:
        return
    raise AssertionError(
        f"this test left a telemetry exporter thread running: {sorted(fresh)}. A "
        "metrics provider must be shut down for real (reset_for_testing()), and a "
        "test that stubs `shutdown` must delegate to it -- see "
