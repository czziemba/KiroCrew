...
- **It does not trust `build-windows`'s job result.** That caller runs with
  ...
  probe checks the listing's own exit status separately from the match, so a
  transient API error fails the step instead of being laundered into "nothing to
  publish". Asking the Actions API what this run uploaded needs `actions: read`,
