...
def _certificate_pems(blob: bytes) -> list[str]:
    with tempfile.NamedTemporaryFile(suffix=".p7b.der") as handle:
        handle.write(blob)
        handle.flush()
        try:
            pem = _openssl(
                ["pkcs7", "-inform", "DER", "-in", handle.name, "-print_certs", "-outform", "PEM"]
            )
        except VerificationError as exc:
            raise VerificationError(
                f"the signature blob is not parseable as PKCS#7: {exc}"
            ) from exc
