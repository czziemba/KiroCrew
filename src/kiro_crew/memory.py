...
class MemoryStore:
    ...
    def write_preferences(self, content: str, *, expected_baseline: str | None = None) -> bool:
        ...
        no longer matches it (checked inside the lock), the write is skipped
        and ``False`` is returned. ``None`` writes unconditionally (direct
        user intent wins). Returns ``True`` when the write happened.
        """
        self._memory_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            with file_lock(lock_fd, exclusive=True):
                if (
                    ...
                    and self.read_preferences().strip() != expected_baseline.strip()
                ):
                    logger.info(
    ...
    def write_projects(self, content: str, *, expected_baseline: str | None = None) -> bool:
        ...
        try:
            with file_lock(lock_fd, exclusive=True):
                if (
                    expected_baseline is not None
                    and self.read_projects().strip() != expected_baseline.strip()
                ):
                    logger.info(
    ...
    def read_history_entries(self, since: _date | None = None) -> list[dict]:
        ...
        for day, f in candidates:
            ...
            # A glob-enumerated file exists, so a missing updated_at means the
            # guarded read refused it (e.g. a planted symlink) — skip it.
