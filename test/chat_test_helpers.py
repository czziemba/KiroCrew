...
async def drain_background_tasks(state) -> None:
    ...
    Slack link-time backfill is the one this exists for. A test that asserts on
    what such a task did has to await it. Exiting the ``TestClient`` block is not
    enough: whether the loop happens to run the task on the way out depends on how
    busy the machine is, so on a loaded CI shard the assertion sees a mock that was
    never awaited and reports ``'NoneType' object has no attribute 'args'`` — a
    DIFFERENT test in the class each run, which reads as "the suite is flaky"
    rather than as one missing await (#4130).
