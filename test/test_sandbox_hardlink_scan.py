...
def _run_scan(*, dirs: list[str], files: list[str]) -> tuple[int, str | None]:
    """Execute the scan with *dirs*/*files* as the protected paths.
    ...
    try:
        exec(_scan_source(), namespace)  # the code under test IS this source
...
class TestTheGateOnWhetherToWalkAtAll:
    def test_a_directory_named_as_a_credential_file_does_not_arm_the_scan(
        ...
        walked, refusal = _run_scan(dirs=[], files=[str(cred_dir), str(tmp_path / "absent")])
    ...
    def test_a_single_linked_credential_does_not_arm_the_scan(self, tmp_path: Path) -> None:
        ...
        walked, refusal = _run_scan(dirs=[], files=[str(cred)])
        assert walked == 0
        assert refusal is None
    ...
    def test_a_symlink_to_a_directory_does_not_arm_the_scan(self, tmp_path: Path) -> None:
        ...
        walked, refusal = _run_scan(dirs=[], files=[str(link)])
        assert walked == 0
        assert refusal is None
...
class TestTheRefusalStillFires:
    ...
    def test_a_credential_inside_a_protected_DIR_also_arms_it(self, tmp_path: Path) -> None:
        ...
        walked, refusal = _run_scan(dirs=[str(cred_dir)], files=[])
        assert walked > 0
        assert refusal is not None
        assert "BLOCKED" in refusal
